...
function Vc() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/leave/evaluates/study/pendingApprovalsVC", {
  ...
  const openStudyLeaveFormPage = (leaveId, evaluation_type) => {
    navigate('/noc/studyLeaveDetailsToVC', { state: { id: leaveId, evaluation_type: evaluation_type } });
  };
