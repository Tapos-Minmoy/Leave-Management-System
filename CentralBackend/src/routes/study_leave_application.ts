...
studyLeaveApplicationRouter.get("/", async (req, res) => {
  try {
    var query = db.selectFrom("Study_Leave_Application").selectAll();
...
studyLeaveApplicationRouter.post("/add", async (req, res) => {
  try {
    const {
      ...
      program_start_date,
      signature,
    } = studyLeaveReqBody.parse(req.body);
    ...
    // Add request to Study_Leave_Evaluation table
    await db
    ...
    res.status(200).send({
      message: "Data Inserted Successfully in Study_Leave_Application and Study_Leave_Evaluation Tables.",
    });
  } catch (error) {
