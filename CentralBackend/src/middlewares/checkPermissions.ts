...
export async function checkPermissions(
  ...
  try {
    ...
    const isStudent = await db.selectFrom("Student").where("user_id", "=", uid)
    ...
    const isTeacher = await db.selectFrom("Teacher").where("user_id", "=", uid)
    ...
    const isStaff = await db.selectFrom("Roles").where("user_id", "=", uid)
